				...
				<ul class="post-list">
					<?php while(have_posts()) : the_post(); $type = get_post_type_object(get_post_type()) ?>
						<li <?php post_class(array('post')) ?>>
							...
							elseif($post->post_type == 'client' && get_post_meta($post->ID, 'client_url', true)) { ?><a href="<?php print esc_url(get_post_meta($post->ID, 'client_url', true)) ?>"><?php }
							?>
								...
								<?php endif; ?>
							<?php if($type->public || ($post->post_type == 'client' && get_post_meta($post->ID, 'client_url', true))) print '</a>' ?>
