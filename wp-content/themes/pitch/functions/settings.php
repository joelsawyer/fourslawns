...
function pitch_settings_admin_init(){
	...
	add_settings_field('menu_extras', __('Menu Extras', 'pitch'), 'pitch_settings_field', 'theme_settings', 'general', array(
		'field' => 'menu_extras',
		'type' => 'checkbox',
		'description' => __("Display blog, home and project links in the menus.", 'pitch')
	));

	add_settings_field('topbar_menu', __('Display Top Bar Menu', 'pitch'), 'pitch_settings_field', 'theme_settings', 'general', array(
		'field' => 'topbar_menu',
		'type' => 'checkbox',
		'description' => __("Display the menu bar along the top of Pitch.", 'pitch')
	));
	
	add_settings_field('scale_main_menu', __('Scale Main Menu', 'pitch'), 'pitch_settings_field', 'theme_settings', 'general', array(
		'field' => 'scale_main_menu',
		'type' => 'checkbox',
	...
	add_settings_field('project_url_slug', __('Project Slug', 'pitch'), 'pitch_settings_field', 'theme_settings', 'project', array(
	...
	add_settings_field('footer_text', __('Footer Text', 'pitch'), 'pitch_settings_field', 'theme_settings', 'site_text', array(
		'field' => 'footer_text',
		'type' => 'text',
	));
...
function pitch_theme_settings_refresh_rewrite(){
	...
	if(get_option('pitch_theme_settings_flush_rewrite_rules', false)){
		flush_rewrite_rules();
		delete_option('pitch_theme_settings_flush_rewrite_rules');
	}
}
add_action('load-appearance_page_theme_settings_page', 'pitch_theme_settings_refresh_rewrite');

/**
 * Validate the new theme settings.
 * 
 * @param $values
 * @return array
 */
function pitch_theme_settings_validate($values){
	...
	$current = (array) get_option('pitch_theme_settings', array());
	if(empty($current['project_url_slug']) || $current['project_url_slug'] != $values['project_url_slug']){
		// Calling flush_rewrite_rules here doesn't work because we've already registered the post type
		// We'll enqueue a flush for the next time the settings page is loaded
		update_option('pitch_theme_settings_flush_rewrite_rules', true);
	}
...
function pitch_settings_default_values($defaults){
	$defaults = array_merge($defaults, array(
		...
		// Site Text
		'footer_text' => __('Copyright {sitename}', 'pitch'),
...
function pitch_settings_field($args){
	...
	switch($args['type']){
		...
		case 'dropdown' :
			?>
			<select id="<?php print esc_attr($args['field']) ?>" name="pitch_theme_settings[<?php print $args['field'] ?>]">
